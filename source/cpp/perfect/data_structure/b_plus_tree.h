...
class b_plus_tree : public search_tree<T> {
  struct node {
    vector<T*> key;
    union son {
      vector<node*> inner;
      vector<vector<T>*> leaf;
    }
  };
 ...
 public:
  ...
  b_plus_tree(const b_plus_tree<T, N>& rhs) { copy_self(root, rhs.root); }
  b_plus_tree<T, N> operator=(const b_plus_tree<T, N>& rhs) {
    delete_self(root);
    copy_self(root, rhs.root);
    return *this;
  }
  ~b_plus_tree() { delete_self(root); }
