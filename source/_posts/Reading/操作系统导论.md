...
下面展示一个使用wait()和signal()实现的父线程等待子线程的情况, 当线程睡眠可以调用wait，当线程想要唤醒在这个条件变量上等待的线程时，可以调用signal，唤醒后不一定马上切换过去运行

```c
wait(Condition *c, Mutex *m); //释放锁并睡眠
signal(Condition *c); // 唤醒在此条件上等待的线程
...
void producer(int loops) {
  for(int i = 0; i < loops; i++) {
    mutex->lock();
    if(queue->fill()) {
      wait(condition, mutex); // 队列满了，无法生产，进入睡眠
    }
    queue->insert(i); // 放入了一个东西
    signal(condition); // 唤醒等待条件的线程
    mutex->unlock();
  }
}
...
void consumer(int loops) {
  for(int i = 0; i < loops; i++) {
    mutex->lock();
    if(queue->empty()) {
      wait(condition, mutex); // 队列空了， 无法消费，进入睡眠
    }
...
void producer(int loops) {
  for(int i = 0; i < loops; i++) {
    mutex->lock();
    while (queue->fill()) {
      wait(fill, mutex); // 队列满了，无法生产，进入睡眠
    }
    queue->insert(i); // 放入了一个东西
    signal(emtpy); // 唤醒等待条件的线程
    mutex->unlock();
  }
}
...
void consumer(int loops) {
  for(int i = 0; i < loops; i++) {
    mutex->lock();
    while (queue->empty()) {
      wait(empty, mutex); // 队列空了， 无法消费，进入睡眠
    }
    queue->remove(); // 从队列中取出元素，消费
    signal(fill); // 唤醒等待条件的线程
    mutex->unlock();
  }
}
```
