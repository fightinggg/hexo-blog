...
1. 最开始有一个状态栈，其中包含状态1，一个符号栈，为空， `[状态栈底,1], [符号栈底]`

2. 从状态栈顶读取状态，现在位于状态1， 读入字符`number1`, 执行`s2`, 则状态2入栈，字符number1入栈 ，`[状态栈底,1,2], [符号栈底,number1]`
3. 从状态栈顶读取状态，现在位于状态2， 遇到字符`+`不读入, 执行`r(MUL->number)`, 状态栈弹出状态2，符号栈弹出`number1`，然后`MUL`符号准备入栈, 入栈时状态栈顶为1，于是执行`goto4`, 最终`[状态栈底,1,4], [符号栈底,MUL]`
4. 从状态栈顶读取状态，现在位于状态4，遇到字符`+`不读入, 执行`r(PRODUCTION -> MUL)`， 状态4弹出，符号MUL弹出，然后PRODUCTION符号准备入栈，此时栈顶为1， 于是执行`goto5`，最终`[状态栈底,1,5], [符号栈底,PRODUCTION]`
5. 从状态栈顶读取状态，现在位于状态4，读入字符`+`， 执行`s8`,最终`[状态栈底,1,5,8], [符号栈底,PRODUCTION,+]`
6. 从状态栈顶读取状态，现在位于状态8，读入字符`number2`,执行`s10`,最终`[状态栈底,1,5,8,10], [符号栈底,PRODUCTION,+,number2]`
7. 从状态栈顶读取状态，现在位于状态10，遇到字符`*`不读入,于是弹栈、`goto9`, 最终最终`[状态栈底,1,5,8,9], [符号栈底,PRODUCTION,+,MUL]`
8. 从状态栈顶读取状态，现在位于状态9, 读入字符`*`，执行`s6`, 最终`[状态栈底,1,5,8,9,6], [符号栈底,PRODUCTION,+,MUL,*]`
9. 从状态栈顶读取状态，现在位于状态6, 读入字符`number3`， 执行`s7`， 最终`[状态栈底,1,5,8,9,6,7], [符号栈底,PRODUCTION,+,MUL,*,number3]`
10. 从状态栈顶读取状态，现在位于状态7,遇到字符`$`不读入,执行`r(MUL -> MUL * number )`, 符号栈弹出三个，状态栈弹出三个，然后处于状态8，按照发现遇到MUL执行`goto9`, 于是最终`[状态栈底,1,5,8,9], [符号栈底,PRODUCTION,+,MUL]`
11. 从状态栈顶读取状态，现在位于状态9,遇到字符`$`不读入, 执行`r(SUM -> PRODUCTION + MUL)`，两个栈都是弹出三个，处于状态1，遇到SUM,执行`goto3`,最终`[状态栈底,1,3], [符号栈底,SUM]`
12. 从状态栈顶读取状态，现在位于状态3,读入字符`$`，结束
